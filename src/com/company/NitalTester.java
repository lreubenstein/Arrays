...
public class NitalTester {
    ...
    public static boolean isNital(int[][] square){
        // check first row for repeats
//        int [] row1 = square[0];
//        if(containsRepeats(row1)) return false;
//
//        // check each row
//        for(int r = 1; r < square.length; r++){
//            if(!hasAllValues(row1, square[r]))return false;
//        }
//
//        // check each column
//        for(int c = 0; c < row1.length; c++){
//            int [] col = getColumn(square, c);
//            if(!hasAllValues(row1, col)) return false;
//        }
//        return true;
        int [] fcol = getColumn(square, 0);
        for(int i = 0; i < square.length; i++){
            int [] ncol = getColumn(square,i);
            if (containsRepeats(ncol)){
                return false;
            }
