...
public class WordPuzzle {
    ...
    othewise it returns false.  The parameter blackBoxes indicates which
    ...
    public WordPuzzle(boolean [][] blackBoxes){
        ...
        int num = 1;    // start numbering at 1
        for(int r = 0; r < rows; r++){
            for(int c = 0; c < cols; c++){
                boolean lab = toBeLabeled(r,c,blackBoxes);
                boolean black = blackBoxes[r][c];
                if (lab) {
                    puzzle[r][c] = new Box(black, num);
                    num++;
                }
                else{
                    puzzle[r][c] = new Box(black, 0);
                }
            }
        }
    }
